...
class RandomCrop(object):
    ...
    """
    def __init__(self, output_size, black_trim=True):
    ...
    def __call__(self, input):
        ...
        if type(input) == list:
            if black_trim:
                return [
                    im.crop(bbox).crop((left, top, left + self.output_size[0], top + self.output_size[1]))
                    for im in input
                ]
            return [im.crop((left, top, left + self.output_size[0], top + self.output_size[1])) for im in input]
        if black_trim:
            return input.crop(bbox).crop((left, top, left + self.output_size[0], top + self.output_size[1]))
...
class ResizeFixMin(object):
    ...
    def __call__(self, input):
        if type(input) == list:
            w, h = input[0].size
            if w > h:
                return [im.resize((int(self.img_size * (w / h)), self.img_size), Image.BILINEAR) for im in input]
            return [im.resize((self.img_size, int(self.img_size * (h / w))), Image.BILINEAR) for im in input]

        w, h = input.size
        if w > h:
            return input.resize((int(self.img_size * (w / h)), self.img_size), Image.BILINEAR)
        return input.resize((self.img_size, int(self.img_size * (h / w))), Image.BILINEAR)
...
class RandomResizeRange(object):
    ...
    img_size can be an int, list or tuple (width, height)
    """
    def __init__(self, min_img_size, max_img_size=None):
        self.max_img_size = max_img_size
        self.min_img_size = min_img_size

    def get_value(self, input):
        w, h = input.size
        if self.max_img_size is None:
            if w > h:
                max_value = h
            else:
                max_value = w
        else:
            max_value = self.max_img_size
        img_size = torch.randint(self.min_img_size, max_value + 1, (1, )).item()
        return img_size

    def __call__(self, input):
        if type(input) == list:
            w, h = input[0].size
            img_size = self.get_value(input[0])
            if w > h:
                return [im.resize((int(img_size * (w / h)), img_size), Image.BILINEAR) for im in input]
            return [im.resize((img_size, int(img_size * (h / w))), Image.BILINEAR) for im in input]

        w, h = input.size
        img_size = self.get_value(input)
        if w > h:
            return input.resize((int(img_size * (w / h)), img_size), Image.BILINEAR)
        return input.resize((img_size, int(img_size * (h / w))), Image.BILINEAR)
...
class RandomSequenceCrop(object):
    ...
    def __call__(self, input):
        ...
        assert max_start_ind > 0, 'Sequence length longer than input sequence length: ' + \
            str(input_seq_len) + '.'
...
class ResizeSeqMin(object):
    ...
    def __call__(self, input):
        ...
        T, C, H, W = input.shape
        if H < W:
            return F.interpolate(input.float(), size=(self.min_len, int(float(W) / float(H) * self.min_len)))
        else:
            return F.interpolate(input.float(), size=(int(float(H) / float(W) * self.min_len), self.min_len))
